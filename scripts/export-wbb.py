...
def build_markdown(gallery_dir: Path, doc_title: str,
    ...
    # Export dir is where the combined .md will be written (sim_root/export/)
    export_dir = gallery_dir.parent.parent / "export"
    img_re = re.compile(r"(!\[[^\]]*\]\()([^)]+)(\))")

    for chapter_id, path in chapters:
        content = path.read_text().strip()
        content = strip_meta(content)
        # Rewrite image paths: resolve relative to chapter location
        # (gallery/wbb/), then make relative to export dir
        def _rewrite_img(m: re.Match) -> str:
            abs_path = (path.parent / m.group(2)).resolve()
            try:
                rel = os.path.relpath(abs_path, export_dir)
            except ValueError:
                rel = m.group(2)
            return m.group(1) + rel + m.group(3)
        content = img_re.sub(_rewrite_img, content)
