...
<!-- Tobi: Vera-Abstimmung zu Schichtgrenzen ist erledigt (Tag 2 Meeting). Die offene Frage aus v1 ist gelöscht. -->
...
<!-- Tobi: Blueprint-Name von "BP_SchattenfiebertSystem" (Tippfehler im v1) auf "BP_Schattenfieber" korrigiert. -->
...
<!-- Tobi: Kapitelname von "Relikt-Shader" auf "Schwellenanker-Shader" geändert. Asset-Namen konsequent umbenannt. Der Schwellenanker ist das namensgebende Relikt dieser Iteration: RELICS: Schwellenanker. -->
...
**Zur Form**: Vera hat die Geometriebeschreibung auf "folded geological formation, compressed ossified mineral cluster" spezifiziert — weg von einer wirbelsäulenartigen Struktur. Das ist eine Neuigkeit mit Shader-Konsequenzen: SSS-Radius und Riss-Topologie müssen auf eine verdichtete, mineralische Form kalibriert werden, nicht auf eine langgestreckte organische. Der Riss-Procedural-Noise (`T_Schwellenanker_Riss_01`) wird entsprechend neu kalibriert — komprimierte Rissstrukturen statt radialer Brüche.
...
*Designabsicht*: Visuelles Vokabular entspricht bewusst PP-Stufe 3. Der Schwellenanker und das Schattenfieber sprechen dieselbe Sprache: Rissstrukturen, Kalt-Violett, Innenleuchten, Instabilität. Der Spieler sieht diese Verbindung — ob er sie versteht, ist Erzählung. Nami hat darauf hingewiesen, dass ein Anker reißen kann — das muss in Zustand 3 visuell lesbar sein: keine kontrollierte Instabilität, sondern ein Objekt kurz vor dem Bruch.

<!-- Tobi: Namis narrativer Kommentar (Tag 3 Briefing) — "ein Anker kann reißen" — ist für Act 3 relevant. Das ist ein Gameplay-Signal, das ich im Shader bereits angelegt habe: Zustand 3 zeigt Rissstrukturen mit steuerbarem Öffnungsgrad. Das kann für Act 3 sequenziell aufgezogen werden. Keine neue Shader-Arbeit nötig, nur Blueprint-Kurven-Authoring. -->
...
|---|---|---|---|
| ~~Wie diskret sind die Stadtebenen-Übergänge im Level-Design?~~ | Vera | ~~KRITISCH~~ | **ERLEDIGT** (Tag 2): oben fließend, unten diskret |
| Welche Sichtachsen / Constraint-Punkte für Houdini-Terrain? | Vera / Emre | Hoch | Offen — wartet auf Topos-Kapitel |
| Wie triggert das Lymph-Subsystem die PP-Stufen? (Interface-Definition) | Darius | Hoch — vor Alpha-Architektur | Offen |
| Braucht Fraktionswahl in Stunde 1 unterschiedliche PP-Presets? | Nami | Mittel — vor Beta | Offen |
| Sind Tiervolk-Siedlungen an festen "dünnen Orten" oder mobil? | Emre | Mittel | Offen — bestimmt ob Ambient-Werte statisch oder dynamisch |

---

*Dokument-Status*: Version 2.0. Schwellenanker-Rename vollständig durchgezogen. Vera-Schichtgrenzen-Frage als erledigt markiert. Form-Kommentar (mineralisch-komprimiert) in Shader-Sektion integriert. Namis Act-3-Anmerkung im Shader-Kommentar verankert.
